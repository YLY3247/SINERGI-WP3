...
N_exp = 30; %The number of independent simulations
T = 1e6; %selling horizon
regret_data_our = zeros(N_exp, T);
data = readmatrix('Data/data_3_4_5.xlsx'); %Read the MLNL model
Num_level_1 = 3; %D1
Num_level_2 = 4; %D2
Num_level_3 = 5; %D3
...
mu_max = 2;
eta_min = 0.5;
U_min = 25; %lower bound of preference weight
U_max = 30; %upper bound of preference weight
...
for n_exp = 1:1:N_exp %conduct the dynamic assortment algorithm for N_exp times
    ...
    while t <= T
        ...
        elseif stage == 2
            %disp("exploitation");
            %disp(cycle);
            M_0 = Num_level_2+Num_level_2*Num_level_3+1;
            T_duration = M_0^2*N_leaf*cycle;  
...
%output the regret
time = 1:1:length(regret_mean);
result = [time; regret_mean];
filename = sprintf('regret_%d_%d_%d.xlsx', Num_level_1, Num_level_2, Num_level_3);
